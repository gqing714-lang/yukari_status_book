/* 虚见相 · 結縁帳状态册 v3.1
 * SillyTavern JS-Slash-Runner / 酒馆助手脚本库用
 * 上传到仓库后，用 loader 脚本 fetch+eval 调用即可。
 */
(() => {
  const ST_WIN = window.parent && window.parent !== window ? window.parent : window;
  ...
  const CONFIG = {
    ...
    storageVisible: 'ykr_status_book_visible_v31',
    storageExpanded: 'ykr_status_book_expanded_v31',
  ...
  const ID = {
    root: 'ykr-status-book-root',
    style: 'ykr-status-book-style',
  };
  ...
  const state = {
    visible: localStorage.getItem(CONFIG.storageVisible) === '1',
  ...
  function splitStatusSections(block) {
    ...
    for (const line of lines) {
      ...
      if (match) {
        current = match[1];
        const value = removeComment(match[2]);
        if (value) sections[current].push(value);
        continue;
      }
      if (current) sections[current].push(removeComment(line));
    }
    return sections;
  }
  ...
  function parseMain(raw) {
    ...
    if (text.includes('|')) {
      ...
      return { title: safeText(title) || '未命名主线', summary: safeText(rest.join('|')) || '暂无梗概。' };
    }
  ...
  function addStyle() {
    ...
    style.textContent = `
      ...
      #${ID.root}[data-mood="高兴"] .ykr-seal{background:radial-gradient(circle at 33% 24%,rgba(255,245,223,.98),rgba(226,184,121,.96) 45%,rgba(151,58,45,.98) 100%);}
      #${ID.root}[data-mood="低落"] .ykr-seal,#${ID.root}[data-mood="危险"] .ykr-seal,#${ID.root}[data-mood="愤怒"] .ykr-seal{background:radial-gradient(circle at 33% 24%,rgba(231,219,205,.96),rgba(153,131,118,.94) 45%,rgba(52,36,39,.98) 100%);}
      ...
      #${ID.root}.ykr-expanded .ykr-detail{display:block;animation:ykrInkIn .24s ease both;}
      #${ID.root}.ykr-expanded .ykr-hint{display:none;}
      ...
      .ykr-quote.ykr-flash .ykr-quote-text{animation:ykrInkIn .22s ease both;}
      ...
      @media(max-width:520px){#${ID.root}{right:8px;bottom:calc(env(safe-area-inset-bottom,0px) + 70px);width:min(330px,calc(100vw - 16px));}.ykr-inner{padding:17px 15px 14px;}.ykr-seal{right:15px;width:56px;height:56px;}.ykr-name{font-size:19px;}.ykr-quote-text{font-size:12px;}}
    `;
    ST_DOC.head.appendChild(style);
  }

  function makeUI() {
    let root = ST_DOC.getElementById(ID.root);
    if (root) return root;
    root = ST_DOC.createElement('div');
    root.id = ID.root;
    root.innerHTML = `
    ...
    ST_DOC.body.appendChild(root);
    root.querySelector('.ykr-seal').addEventListener('click', event => { event.stopPropagation(); toggleExpanded(); });
    root.querySelector('.ykr-close').addEventListener('click', event => { event.stopPropagation(); setVisible(false); });
  ...
  function cycleQuote() {
    ...
    const quote = ST_DOC.querySelector(`#${ID.root} .ykr-quote`);
  ...
  function render(nextData) {
    if (nextData) state.data = { ...CONFIG.fallback, ...nextData };
    const root = makeUI();
    ...
    setText(`#${ID.root} .ykr-place`, data.place);
    setText(`#${ID.root} .ykr-time`, data.time);
    setText(`#${ID.root} .ykr-name`, data.name);
    setText(`#${ID.root} .ykr-mood-label`, mood);
    setText(`#${ID.root} .ykr-mood-value`, `${moodValue}`);
    setText(`#${ID.root} .ykr-quote-text`, `「${data.quote || '……'}」`);
    setText(`#${ID.root} .ykr-outfit`, data.outfit);
    setText(`#${ID.root} .ykr-action`, data.action);
    setText(`#${ID.root} .ykr-main-title`, data.mainTitle);
    setText(`#${ID.root} .ykr-main-summary`, data.mainSummary);
    ...
    if (todoList) {
      ...
      for (const todo of todos) {
        const li = ST_DOC.createElement('li');
        li.textContent = todo;
  ...
  function registerScriptButton() {
    try {
      if (typeof G.appendInexistentScriptButtons === 'function') {
        G.appendInexistentScriptButtons([{ name: CONFIG.buttonName, visible: true }]);
      }
      if (typeof G.eventOn === 'function' && typeof G.getButtonEvent === 'function') {
        const stop = G.eventOn(G.getButtonEvent(CONFIG.buttonName), () => toggleVisible());
        if (stop?.stop) state.stops.push(stop);
      }
    } catch (error) {
  ...
  async function getLastMessageIdSafe() {
    try {
      if (typeof G.getLastMessageId === 'function') {
        const id = Number(G.getLastMessageId());
        if (!Number.isNaN(id)) return id;
      }
    } catch (_) {}
    try {
      if (typeof G.triggerSlash === 'function') {
        const id = Number(await G.triggerSlash('/pass {{lastMessageId}}'));
        if (!Number.isNaN(id)) return id;
      }
    } catch (_) {}
    try {
      const chat = ST_WIN.SillyTavern?.getContext?.()?.chat;
  ...
  async function getLatestAssistantMessageFromHelper() {
    if (typeof G.getChatMessages !== 'function') return null;
    try {
      const lastId = await getLastMessageIdSafe();
      const messages = await Promise.resolve(G.getChatMessages(`0-${lastId}`, {
        role: 'assistant',
        hide_state: 'unhidden',
      ...
      for (let i = messages.length - 1; i >= 0; i--) {
        ...
        if (block) return { messageId: msg.message_id ?? i, message: msg.message, block };
      }
    } catch (error) {
  ...
  function getLatestStatusFromDom() {
    const nodes = [...ST_DOC.querySelectorAll('#chat .mes_text')].reverse();
  ...
  function bindEvents() {
    try {
      if (typeof G.eventOn === 'function' && typeof G.tavern_events !== 'undefined') {
        const names = ['MESSAGE_RECEIVED', 'MESSAGE_UPDATED', 'MESSAGE_SWIPED', 'CHAT_CHANGED', 'GENERATION_ENDED'];
        for (const name of names) {
          if (G.tavern_events[name]) {
            const stop = G.eventOn(G.tavern_events[name], () => scheduleUpdate(520));
            if (stop?.stop) state.stops.push(stop);
          }
        }
      }
  ...
  function bindDomObserverFallback() {
    const chat = ST_DOC.querySelector('#chat');
    if (!chat || state.observer) return;
    state.observer = new ST_WIN.MutationObserver(() => scheduleUpdate(700));
  ...
  function hideStatusBlocksInChat() {
    ...
    const nodes = ST_DOC.querySelectorAll('#chat .mes_text');
  ...
  function init() {
    ...
    scheduleUpdate(260);
    try {
      if (typeof G.replaceScriptInfo === 'function') {
        G.replaceScriptInfo('虚见相 · 結縁帳状态册：读取最新 <status> 状态块，自动更新和风状态栏。');
      }
    } catch (_) {}
  }
  ...
  function destroy() {
    ...
    for (const item of state.stops) {
      try { item?.stop?.(); } catch (_) {}
    }
    state.stops = [];
    ST_DOC.getElementById(ID.root)?.remove();
    ST_DOC.getElementById(ID.style)?.remove();
  }
