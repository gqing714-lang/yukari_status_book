(function () {
  ...
  function buildConfigPanel(root) {
    if (doc.getElementById(CONFIG_PANEL_ID)) return;
  ...
  function toggleConfigPanel() {
    const now = Date.now();
    if (now - state.configToggleStamp < 120) return;
    state.configToggleStamp = now;
    try { win.__ykrVnStatusConfigToggleStamp = now; } catch (e) {}

    const panel = doc.getElementById(CONFIG_PANEL_ID);
    if (panel) panel.classList.toggle('is-open');
  }

  function findHelperFunction(name) {
    try {
      if (name === 'appendInexistentScriptButtons' && typeof appendInexistentScriptButtons === 'function') return appendInexistentScriptButtons;
      if (name === 'updateScriptButtonsWith' && typeof updateScriptButtonsWith === 'function') return updateScriptButtonsWith;
      if (name === 'replaceScriptButtons' && typeof replaceScriptButtons === 'function') return replaceScriptButtons;
      if (name === 'eventOnButton' && typeof eventOnButton === 'function') return eventOnButton;
      ...
      if (name === 'getScriptId' && typeof getScriptId === 'function') return getScriptId;
    } catch (e) {}

    const sources = [];
    try { sources.push(window); } catch (e) {}
    try { if (window.TavernHelper) sources.push(window.TavernHelper); } catch (e) {}
    try { if (window.parent && window.parent !== window) sources.push(window.parent); } catch (e) {}
    try { if (window.parent?.TavernHelper) sources.push(window.parent.TavernHelper); } catch (e) {}
  ...
  function getScriptIdSafe() {
    const getId = findHelperFunction('getScriptId');
    if (!getId) return undefined;
    try { return getId(); } catch (e) { return undefined; }
  }

  function callButtonListFunction(fn, buttons) {
    if (!fn) return false;
    try {
      fn(buttons);
      return true;
  ...
  function registerHelperScriptButton() {
    const buttons = [{ name: SCRIPT_BUTTON_NAME, visible: true }];
    let buttonAdded = false;

    const appendButtons = findHelperFunction('appendInexistentScriptButtons');
    if (appendButtons) buttonAdded = callButtonListFunction(appendButtons, buttons);

    if (!buttonAdded) {
      const updateButtons = findHelperFunction('updateScriptButtonsWith');
      if (updateButtons) {
        try {
          updateButtons(list => {
            const current = Array.isArray(list) ? list : [];
            return current.some(btn => btn && btn.name === SCRIPT_BUTTON_NAME)
              ? current.map(btn => btn && btn.name === SCRIPT_BUTTON_NAME ? { ...btn, visible: true } : btn)
              : [...current, ...buttons];
          });
          buttonAdded = true;
        } catch (e1) {
    ...
    if (!buttonAdded) {
      const replaceButtons = findHelperFunction('replaceScriptButtons');
      if (replaceButtons) buttonAdded = callButtonListFunction(replaceButtons, buttons);
    }

    const handler = () => toggleConfigPanel();
    let eventBound = false;

    const onButton = findHelperFunction('eventOnButton');
    if (onButton) {
      try {
        onButton(SCRIPT_BUTTON_NAME, handler);
        eventBound = true;
      } catch (e1) {
        const scriptId = getScriptIdSafe();
        if (scriptId != null) {
          try {
            onButton(scriptId, SCRIPT_BUTTON_NAME, handler);
            eventBound = true;
          } catch (e2) {}
        }
      }
    ...
    if (!eventBound) {
      const eventOnFn = findHelperFunction('eventOn');
      const getButtonEventFn = findHelperFunction('getButtonEvent');
      if (eventOnFn && getButtonEventFn) {
        try {
          eventOnFn(getButtonEventFn(SCRIPT_BUTTON_NAME), handler);
          eventBound = true;
        } catch (e1) {
          const scriptId = getScriptIdSafe();
          if (scriptId != null) {
            try {
              eventOnFn(getButtonEventFn(scriptId, SCRIPT_BUTTON_NAME), handler);
              eventBound = true;
            } catch (e2) {}
          }
        }
    ...
    if (!buttonAdded || !eventBound) {
      console.warn('[yukari-vn-status] 未找到完整的酒馆助手脚本按钮 API，配色面板按钮未能注册。');
    }
  }
